...
export namespace UninstallMCProfile {
    export class Writer extends UninstallTracker.Writer {
        ...
        protected async writeSpecific() {
            await this.writeString(this.profileId);
        }
    }
    ...
    export class Reader extends UninstallTracker.Reader {
        ...
        public async uninstall() {
            ...
            if (!minecraftDir) return; // do not insist on getting minecraft dir, because a previous version could have corruped the input map. Uninstallation should always be possible

            const profilesFile = await getMinecraftLauncherProfiles(minecraftDir);
            if (!await exists(profilesFile)) return; // profiles files does not exist
